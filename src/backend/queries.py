...
def get_trending_videos():
    ...
    if not videos:
        videos = (
            Video.objects.public()
            .order_by("-bunnyvideo__views_gained")
            .filter(bunnyvideo__views_gained__gt=1)
        )
        videos = list(videos[0: min(videos.count(), 50)])
        for v in videos:
            setattr(v, "thumbnail_url", v.get_thumbnail())
...
def get_video(watch_id) -> Video | None:
    try:
...
def get_videos_from_channel(channel):
    return Video.objects.public().filter(
        channel__exact=channel, visibility__exact=Video.VisibilityStatus.PUBLIC
    )
...
def get_sub_feed(channel):
    ...
    if not videos:
        ...
        videos = Video.objects.filter(
            channel__in=[sub.to_channel for sub in subs.all()]
        ).order_by("-created")
        for v in videos:
            setattr(v, "thumbnail_url", v.get_thumbnail())
...
def toggle_dislike(to_video, from_channel):
    ...
    dislike = Dislikes.objects.filter(
        channel__exact=from_channel, video__exact=to_video
    )
    if dislike.exists():
        dislike[0].delete()
        return (_get_likes(to_video), _get_dislikes(to_video))
...
def is_video_disliked(to_video, from_channel):
    return Dislikes.objects.filter(
        channel__exact=from_channel, video__exact=to_video
    ).exists()
...
def toggle_subscription(to_channel, from_channel):
    sub = Subscription.objects.filter(
        to_channel__exact=to_channel, from_channel__exact=from_channel
    )
    if sub.exists():
        sub[0].delete()
    else:
...
def is_comment_liked(comment, channel):
    return CommentLike.objects.filter(
        comment__exact=comment, channel__exact=channel
    ).exists()
