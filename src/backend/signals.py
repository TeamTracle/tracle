...
def generate_channel_id(sender, instance, **kwargs):
    if not instance.channel_id:
        instance.channel_id = base64.urlsafe_b64encode(
            fixedint.Int64(hash(str(instance.id + 1))).to_bytes()
        ).decode("UTF-8")[:-1]
        instance.save()
...
def generate_watch_id(sender, instance, created, **kwargs):
    ...
    if not instance.watch_id:
        instance.watch_id = base64.urlsafe_b64encode(
            fixedint.Int64(hash(str(instance.id))).to_bytes()
        ).decode("UTF-8")[:-1]
        try:
        ...
        try:
            ...
            if (
                not instance.subs_notified
                and instance.published
                and status == BunnyVideo.TranscodeStatus.DONE
                and instance.visibility == instance.VisibilityStatus.PUBLIC
            ):
                action.send(instance.channel, verb="uploaded", action_object=instance)
                subs = [
                    sub.from_channel.user
                    for sub in instance.channel.subscriptions.all()
                ]
                for sub in subs:
                    Notification.objects.create(
                        notification_type=Notification.NotificationType.VIDEO,
                        actor=instance.channel,
                        action_object=instance,
                        target_object=instance.channel,
                        recipient=sub,
                    )
...
def send_comment_notification(sender, instance, created, **kwargs):
    if created:
        ...
        tags = re.findall("@\(\w+\)\[([aA-zZ0-9-_]+)\]", instance.text)  # noqa W605
        if tags:
