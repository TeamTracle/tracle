...
class VideoManager(models.Manager):
    ...
    def public(self):
        ...
        qs = qs.filter(
            visibility="PUBLIC",
            published=True,
            channel__user__banned=False,
            videostrike__isnull=True,
        )
        qs = qs.filter(age_restricted=False)
    ...
    def search(self, query):
        ...
        similarity = (
            ...
            + TrigramSimilarity("channel__name", query)
        )
        return (
            qs.annotate(rank=sr, similarity=similarity)
            .annotate(score=(models.F("rank") + models.F("similarity")) / 2)
            .filter(score__gt=0.1)
            .order_by("-score")
        )
