...
@register.simple_tag
def register_notify_callbacks(badge_class='nav__notifications__badge',  # pylint: disable=too-many-arguments,missing-docstring
...
def live_notify_badge(context, badge_class='nav__notifications__badge'):
    ...
    html = "<span class='{badge_class}'>{unread}</span>".format(
        badge_class=badge_class, unread=user.notifications.unread().filter(recipient=user).count()
    )
